...
const racaOpts = [{ value: 'branca', label: 'Branca' }, { value: 'parda', label: 'Parda' }, { value: 'preta', label: 'Preta' }, { value: 'amarela', label: 'Amarela' }];
const generoOpts = [{ value: 'masculino', label: 'Masculino' }, { value: 'feminino', label: 'Feminino' }, { value: 'outro', label: 'Outro' }];
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.logoArea}><Logo /><Text style={styles.title}>Faça seu cadastro</Text></View>
        <Input label="Nome completo" value={nome} onChangeText={setNome} placeholder="Nome completo" />
        <Input label="Data de nascimento" value={data} onChangeText={setData} placeholder="DD/MM/AAAA" keyboardType="numeric" />
        <Select label="Cor/ raça" value={raca} options={racaOpts} onSelect={setRaca} />
        <Select label="Gênero" value={genero} options={generoOpts} onSelect={setGenero} />
        ...
        <Button title="Cadastre se" onPress={() => navigation.navigate('Cadastro2')} style={{ marginBottom: spacing.md }} />
